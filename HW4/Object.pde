public class GameObject {
    ...
    void debugDraw() {
        ...
        for (int i = 0; i < mesh.triangles.size(); i++) {
             ...
             Vector3 normal = Vector3.cross(img_pos[1].sub(img_pos[0]), img_pos[2].sub(img_pos[0]));
             normal.normalize();

             // Calculate the view direction
             Vector3 viewDir = main_camera.transform.position.sub(img_pos[0]);
             viewDir.normalize();
             ...
             // Perform backface culling
              if (dotProduct < 0.0) {
                 // The triangle is facing the camera, render it
